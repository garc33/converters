...
public class ArrayObjectConverter<T> extends SimpleObjectConverter<T[]> {
	...
	public ArrayObjectConverter(ObjectConverter<T> elementConverter, Class<T> type) {
		this(elementConverter, type, ConverterConstants.DEFAULT_SEPARATOR);
	}

	public ArrayObjectConverter(ObjectConverter<T> elementConverter, Class<T> type, final char separator) {
	...
	protected T[] doConvert(String s) throws ConverterException {
		...
		final Object result = Array.newInstance(ClassUtils.primitiveToWrapper(type), length);
